...
MODEL_PATH = os.getenv("MODEL_PATH", "/app/harmful_content_guard.pth")
METRICS_PATH = os.getenv("METRICS_PATH", "/app/harmful_content_guard_metrics.json")
...
class PredictResponse(BaseModel):
    ...
    reason: str  # "safe" | "harmful-content"
...
def predict(req: PredictRequest):
    ...
    # harmful content guard: high confidence = harmful (inverted from food boundary guard)
    if confidence >= threshold:
        decision, reason = "rejected", "harmful-content"
    else:
        decision, reason = "accepted", "safe"

    return PredictResponse(
        request_id=req_id,
        guard_model="harmful_content_guard",
